"""Adapted from Taxim by @Si-Lynnn at https://github.com/CMURoboTouch/Taxim.

Label circles in raw RGB images and save the results to a csv file.
CSV columns include: image_file_path, touch center, touch radius
...
parser.add_argument("-data_path", nargs='?', default=f'{base_path}/images',
                    help="Path to the collected raw tactile images")
...
class CalibrateApp(ng.Screen):
  ...
  def __init__(self, data_path):
    super(CalibrateApp, self).__init__((1024, 768), "Digit Calibration App")
    ...
    def cb():
        ...
        print(f"Selected directory = {self.img_data_dir}")
    ...
    def cb():
      out_fn_path = osp.join(self.img_data_dir, "dataPack.csv")
      print("Saving params to %s"%out_fn_path)
      with open(out_fn_path, "w") as f:
        writer = csv.writer(f)
        writer.writerow(['bg_img_path', 'img_name', 'center', 'radius'])
        for i in range(len(self.imgs)):
          writer.writerow([self.bg_img_fn, self.names[i], self.touch_centers[i], int(self.touch_radius[i])])
      print("Saved!")
    b.set_callback(cb)
